<?php
/**
 * owntheme functions and definitions
 *
...
if ( ! function_exists( 'owntheme_setup' ) ) :
	/**
	...
	function owntheme_setup() {
		 ...
		 * If you're building a theme based on owntheme, use a find and replace
		 * to change 'owntheme' to the name of your theme in all the template files.
		 */
		load_theme_textdomain( 'owntheme', get_template_directory() . '/languages' );
		...
		register_nav_menus(
			array(
				'menu-1' => esc_html__( 'Primary', 'owntheme' ),
			)
		);
		...
		add_theme_support(
			'custom-background',
			apply_filters(
				'owntheme_custom_background_args',
				array(
 ...
 */
function owntheme_widgets_init() {
	register_sidebar(
		array(
			'name'          => esc_html__( 'Sidebar', 'owntheme' ),
			'id'            => 'sidebar-1',
			'description'   => esc_html__( 'Add widgets here.', 'owntheme' ),
